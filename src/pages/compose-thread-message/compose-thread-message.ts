...
@Component({
  selector: 'page-compose-thread-message',
  templateUrl: 'compose-thread-message.html',
})
export class ComposeThreadMessagePage {
  ...
  constructor(
      ...
      composeThread(tempBroadcast: Broadcast) {
        if(tempBroadcast.text == null || tempBroadcast.text == '') {
          ...
          var numOfCommentsRef = firebase.database().ref('/organization/'+this.user.organization_ID+'/message/'+this.broadcastKey+'/numOfComments');
          ...
          this.firebaseService.addCommentToMessage(tempBroadcast,this.user.organization_ID, this.broadcastKey);
          var updates = {};
          updates['/organization/'+this.user.organization_ID+'/message/'+this.broadcastKey+'/numOfComments/'] = currentNumOfComments + 1;
